...
int multiplyMatrices(int matrix1[N][N], int matrix2[N][N], int result[N][N])
{ // parallel matrix multiply with 2d array
    int i, j, k;
...
int multiply1DimensionMatrices(int matrix1[N * N], int matrix2[N * N], int result[N][N])
{ // parallel matrix multiply with 1d array
    int i, j, k;
#pragma omp parallel shared(matrix1, matrix2, result) private(i, j, k)
#pragma omp for schedule(static)
    for (i = 0; i < N; i++)
...
int twoDimToOneDim(int A[N][N], int B[N][N], int dimension, int A1[N * N], int B1[N * N])
...
int main()
{
    ...
    int A1[N * N];
    int B1[N * N];
    zeroMatrix(C); // contain the result
    srand(3);
